...
Every release ships `@lfg-dev/protocol`, `@lfg-dev/client`, `@lfg-dev/react`,
and `@lfg-dev/app` — the last being the exact full application the standalone
web UI runs. These are **not on npm**; they are attached to each GitHub release
as `lfg-dev-<name>-<version>.tgz` tarballs and installed by URL, so no private
registry is required. (The `@lfg-dev` scope is retained deliberately: renaming a
published package name would break existing consumers.) React hosts mount them
with their own transport and asset origin. See
