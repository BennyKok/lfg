...
An HTML artifact can keep one stable card while OMG refreshes its contents from
the server. The executable script must live inside the owning session's cwd and
must print one complete, self-contained HTML document to stdout. OMG passes an
...
Then call `omg_publish_artifact` from the owning session:
...
Use `omg_refresh_artifact` with `{"id":"omg-user-growth","action":"now"}`
...
button, or call `omg_delete_artifact` with its stable `id`. Deletion is limited
