...
const workerQueue: TranscriptIndexWorkerRequest[] = [];
let workerDraining = false;
let workerClosing = false;

function serializeError(err: unknown): { error: string; code?: string } {
  return {
    error: err instanceof Error ? err.message : String(err),
...
function respond(message: TranscriptIndexWorkerResponse): void {
  postMessage(message);
}

async function handleWorkerRequest(req: TranscriptIndexWorkerRequest): Promise<void> {
  switch (req.type) {
    case "cursor": {
      const cursor = transcriptCursorFor(req.path);
      respond({ id: req.id, ok: true, result: cursor, cursor });
      return;
    }
    case "delete": {
      deleteTranscriptIndexForPath(req.path);
      respond({ id: req.id, ok: true, result: null, cursor: null });
      return;
    }
    case "indexMessages": {
      const indexed = indexTranscriptMessages(req.path, req.sessionId, req.lines, req.cursor);
      respond({ id: req.id, ok: true, result: { indexed }, cursor: transcriptCursorFor(req.path) });
      return;
    }
    case "indexTranscript": {
      const result = await indexTranscript(req.path, req.sessionId);
      respond({ id: req.id, ok: true, result, cursor: transcriptCursorFor(req.path) });
      return;
    }
    case "messagePage": {
      const result = await indexedMessagePage(req.path, req.sessionId, req.opts);
      respond({ id: req.id, ok: true, result, cursor: transcriptCursorFor(req.path) });
      return;
    }
    case "search": {
      const result = await searchTranscriptIndex(req.path, req.sessionId, req.query, req.opts);
      respond({ id: req.id, ok: true, result, cursor: transcriptCursorFor(req.path) });
      return;
    }
    case "searchAll": {
      const result = await searchAllTranscriptIndexes(req.query, req.opts);
...
async function drainWorkerQueue(): Promise<void> {
  if (workerDraining) return;
  workerDraining = true;
  try {
    while (workerQueue.length) {
      const req = workerQueue.shift()!;
      try {
        await handleWorkerRequest(req);
      } catch (err) {
        respond({ id: req.id, ok: false, ...serializeError(err) });
      }
      if (workerClosing) {
    ...
    if (workerQueue.length && !workerClosing) void drainWorkerQueue();
  }
}

const workerGlobal = globalThis as unknown as {
  onmessage: ((event: MessageEvent<TranscriptIndexWorkerRequest>) => void) | null;
};

workerGlobal.onmessage = (event: MessageEvent<TranscriptIndexWorkerRequest>) => {
  if (workerClosing) {
    respond({ id: event.data.id, ok: false, error: "transcript index worker is shutting down" });
    return;
  }
  workerQueue.push(event.data);
